...
def main(argv: Optional[List[str]]) -> int:
    ...
    parser.add_argument('patterns', nargs='*', default=['*'], help='Glob patterns to find')
    parser.prog = 'find'
...
def find_files(patterns: List[str]) -> Iterable[str]:
    ...
    if is_git:
        ...
        tracked_files = subprocess.check_output(['git', 'ls-files', '-z'] + globs).decode('utf-8').split('\0')
        tracked_files = [x for x in tracked_files if len(x) > 0]
        # print('tracked:', tracked_files, file=sys.stderr)

        deleted_files = subprocess.check_output(['git', 'ls-files', '-z', '--deleted'] + globs).decode('utf-8').split('\0')
        deleted_files = {x for x in deleted_files if len(x) > 0}
        # print('deleted:', deleted_files, file=sys.stderr)

        untracked_files = subprocess.check_output(['git', 'ls-files', '-z', '--others', '--exclude-standard'] + globs).decode('utf-8').split('\0')
        untracked_files = [x for x in untracked_files if len(x) > 0]
        # print('untracked:', untracked_files, file=sys.stderr)

        all_files = [x for x in tracked_files if x not in deleted_files] + untracked_files
        ...
        all_files = [x for x in all_files if os.path.isfile(x) or os.path.isfile(os.path.realpath(x))]
        return sorted(all_files)
