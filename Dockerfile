...
# Golang builder #
FROM --platform=$BUILDPLATFORM golang:1.23rc2-bookworm AS go-builder--base
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils >/dev/null && \
    rm -rf /var/lib/apt/lists/*
...
# Rust #
FROM --platform=$BUILDPLATFORM rust:1.80.0-slim-bookworm AS rust--build
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        file >/dev/null && \
...
# NodeJS/NPM #
FROM --platform=$BUILDPLATFORM node:22.5.1-slim AS nodejs--base
WORKDIR /app
...
# Composer #
FROM composer:2.7.7 AS composer-bin--base
...
# Main CLI #
FROM --platform=$BUILDPLATFORM node:22.5.1-slim AS cli--base
WORKDIR /app
