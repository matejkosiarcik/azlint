...
# Rust #
FROM --platform=$BUILDPLATFORM rust:1.77.1-slim-bookworm AS rust-builder
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        file >/dev/null && \
...
# NodeJS/NPM #
FROM --platform=$BUILDPLATFORM node:21.7.2-slim AS nodejs-base
WORKDIR /app
...
# Main CLI #
FROM --platform=$BUILDPLATFORM node:21.7.2-slim AS cli-base
WORKDIR /app
