...
FROM debian:12.10-slim AS bins-aggregator--base
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        file >/dev/null && \
...
FROM --platform=$BUILDPLATFORM debian:12.10-slim AS executable-optimizer--base
WORKDIR /app
...
# Gitman #
FROM --platform=$BUILDPLATFORM debian:12.10-slim AS gitman--base
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM --platform=$BUILDPLATFORM debian:12.10-slim AS directory-optimizer--base
...
# Rust #
FROM --platform=$BUILDPLATFORM debian:12.10-slim AS linters--rust--dependencies
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
# Rust #
FROM --platform=$BUILDPLATFORM rust:1.86.0-slim-bookworm AS linters--rust--build
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        file >/dev/null && \
...
FROM debian:12.10-slim AS linters--circleci--base
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM debian:12.10-slim AS linters--shell--loksh--base
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM debian:12.10-slim AS linters--shell--oksh--base
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
# NodeJS/NPM #
FROM --platform=$BUILDPLATFORM node:23.11.0-slim AS linters--nodejs--base
WORKDIR /app
...
FROM debian:12.10-slim AS linters--nodejs--final
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        nodejs npm >/dev/null && \
...
FROM debian:12.10-slim AS rbenv--install
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM debian:12.10-slim AS linters--ruby--base
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM debian:12.10-slim AS linters--ruby--final
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        libyaml-0-2 libyaml-dev build-essential >/dev/null && \
...
# Python/Pip #
FROM debian:12.10-slim AS linters--python--base
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM debian:12.10-slim AS linters--python--final
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
# Composer #
FROM composer:2.8.7 AS linters--composer-bin--base

FROM --platform=$BUILDPLATFORM debian:12.10-slim AS linters--composer-bin--optimize
WORKDIR /app
COPY --from=linters--composer-bin--base /usr/bin/composer ./bin/
# TODO: optimize `composer` script

# PHP/Composer #
FROM debian:12.10-slim AS linters--composer-vendor--base
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM debian:12.10-slim AS linters--composer--final
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM --platform=$BUILDPLATFORM debian:12.10-slim AS linters--brew--install
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM debian:12.10-slim AS linters--brew--rbenv--install
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        autoconf bison build-essential ca-certificates curl git moreutils \
...
FROM --platform=$BUILDPLATFORM debian:12.10-slim AS linters--brew--rbenv--link
WORKDIR /app
...
FROM debian:12.10-slim AS brew--trace
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
...
FROM debian:12.10-slim AS linters--brew--final
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        curl git >/dev/null && \
...
# Main CLI #
FROM --platform=$BUILDPLATFORM node:23.11.0-slim AS cli--base
WORKDIR /app
...
FROM --platform=$BUILDPLATFORM debian:12.10-slim AS cli--final
WORKDIR /app
COPY --from=cli--base /app/cli-dist ./cli-dist
COPY --from=cli--optimize /app/node_modules ./node_modules

# AZLint binaries #
FROM --platform=$BUILDPLATFORM debian:12.10-slim AS azlint--bin
WORKDIR /app
