...
# Rust #
FROM --platform=$BUILDPLATFORM rust:1.72.0-slim-bookworm AS rust-builder
WORKDIR /app
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends file >/dev/null && \
    rm -rf /var/lib/apt/lists/*
...
# NodeJS/NPM #
FROM --platform=$BUILDPLATFORM node:20.6.1-slim AS nodejs-base
WORKDIR /app
...
# Composer #
FROM composer:2.6.3 AS composer-bin
...
# Main CLI #
FROM --platform=$BUILDPLATFORM node:20.6.1-slim AS cli-base
WORKDIR /app
