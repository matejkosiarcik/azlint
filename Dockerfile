...
# Golang builder #
FROM --platform=$BUILDPLATFORM golang:1.22.3-bookworm AS go-builder-base
RUN apt-get update -qq && \
    DEBIAN_FRONTEND=noninteractive DEBCONF_TERSE=yes DEBCONF_NOWARNINGS=yes apt-get install -qq --yes --no-install-recommends \
        moreutils >/dev/null && \
    rm -rf /var/lib/apt/lists/*
...
# NodeJS/NPM #
FROM --platform=$BUILDPLATFORM node:22.2.0-slim AS nodejs-base
WORKDIR /app
...
# Composer #
FROM composer:2.7.6 AS composer-bin
...
# Main CLI #
FROM --platform=$BUILDPLATFORM node:22.2.0-slim AS cli-base
WORKDIR /app
